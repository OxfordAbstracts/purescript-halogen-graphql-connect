module Halogen.GraphQL.Query where
...
-- -- import Halogen.Subscription (Emitter, Finalizer(..), effectEventSource, emit)
...
--         Nothing -> pure unit 
--         Just client -> do
--           q <- H.lift $ query $ Record.delete (Proxy :: _ sym) pass
--           sub <- H.lift $ watchQueryOpts identity queryName q client
--           subId <- H.subscribe $ map QueryUpdate sub
--           H.modify_ _ { subId = Just subId }

--     QueryUpdate (Right res) -> do
--       H.modify_ \st ->  st { pass = Record.set sym (Success res) st.pass }

--     QueryUpdate (Left err) -> do
--       logError $ printJsonDecodeError err
--       H.modify_ \st ->  st { pass = Record.set sym (Failure $ OaGraphqlError $ DecodeError err) st.pass }

--     Receive input -> do
--       { pass } <- H.get
--       let adminGql = Record.get sym pass
--       H.modify_ _ {pass = Record.insert (Proxy :: _ sym) adminGql input }
